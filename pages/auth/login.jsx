...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const {data} = await axios.post('https://education-fijh.onrender.com/auth/login', loginData) // Fetch the data
      setIsSubmit(false) // Turn off loading spinner after getting the response
      Cookies.set('studentToken', data.token, { expires: 1 }); // Expires after 1 days
        ...
        <div className="w-[24rem] mx-4 z-10">
          ...
          <form  onSubmit={handleSubmit} className="card mt-5 rounded-lg p-5 lg:p-7 shadow-lg shadow-[#0000000d] border-neutral-800 border br-card   ">
            ...
            <label className="mb-5 block">
              ...
              <p className="mt-2 text-red-600 hidden" id='phone-alert'>من فضلك ادخل رقم هاتف صحيح !</p>
            </label>
            <label>
              ...
              <span className="relative mt-1.5 flex">
                ...
                <span className=' cfg-main  absolute end-[35px] translate-y-1/2 z-50 ct-2 hover:cursor-pointer' >
                  {isVisible ? (
                    <Eye onClick={toggleVisibility} size={20} />
                  ) : (
                    <EyeSlash onClick={toggleVisibility} size={20} />
                  )}
                </span>
              </span>
            ...
            <div className='flex justify-between'>
              <Link
                ...
                className="ct-2 text-sm mt-3 inline-block hover:text-[#000000c7] dark:hover:text-[#fff]"
              >
                نسيت الرقم السري ؟
              </Link>
              <Link
                target='_blank'
                href="https://wa.me/201061005364"
                className="ct-2 text-sm mt-3 inline-block hover:text-[#000000c7] dark:hover:text-[#fff]"
              >
                تواصل مع الدعم
              </Link>
            ...
            <p className="ct-2 text-sm mt-4 text-center">
              لا تمتلك حساب ؟
              <Link className="ct-primary" href="/auth/signup">
                &ensp; إنشاء حساب
              </Link>
            </p>
          </form>
          <div>
            <p className="text-center text-s ct-3 mt-10 ">
              Developed by <Link href='https://www.facebook.com/mahmoudmagdy47' className='hover:text-[#4f46e5] ct-2' >Mahmoud Magdy</Link> & <Link href='https://www.facebook.com/ahmedashrafaly22' className='ct-2 hover:text-[#4f46e5]'>Ahmed ashraf</Link>
            </p>
          </div>
        </div>
      </main>
